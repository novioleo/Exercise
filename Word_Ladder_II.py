    ...
    def findLadders(self, beginWord, endWord, wordList):
        ...
        def get_next_words(word:str):
            ...
            for i in range(word_len):
                for m_a in alphabet:
                    m_to_add = list_word.copy()
                    if word[i] != m_a:
                        m_to_add[i] = m_a
                        to_return.add(''.join(m_to_add))
            return to_return.intersection(word_set)
        ...
        for m_word in wordList:
            tree[m_word] = set()
            mark_tree[m_word] = word_list_len + 1
        path_results = []
        mark_tree[beginWord] = 0
        # bfs
        next_nodes = get_next_words(beginWord)
        visited_nodes = set()
        count = 1
        while len(next_nodes) != 0:
            visited_nodes = visited_nodes.union(next_nodes)
            new_next_nodes = set()
            for m_next_node in next_nodes:
                tmp = get_next_words(m_next_node)
                tree[m_next_node] = tmp
                new_next_nodes = new_next_nodes.union(tmp)
                if count < mark_tree[m_next_node]:
                    mark_tree[m_next_node] = count
            count += 1
            next_nodes = new_next_nodes.difference(visited_nodes)

        def dfs(path):
            cur = path[0]
            current_depth = mark_tree[cur]
            if current_depth == 0:
                path_results.append(path)
            else:
                for m_next_node in tree[cur]:
                    if current_depth - mark_tree[m_next_node] == 1:
                        t_path = path.copy()
                        t_path.insert(0, m_next_node)
                        dfs(t_path)
